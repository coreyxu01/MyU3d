Shader "MyDemo/WaterWaveEffect"
{
    ...
    {  
        ...
        //abs(x): 绝对值
        //这里是根据时间 控制显示范围的 这个为1的话 就全屏一直显示
        float discardFactor = clamp(_waveWidth - abs(_curWaveDis - dis), 0, 1);  
        //归一化  normalize(x): 归一化， length(x)=1;
        float2 dv1 = normalize(dv);  
        //计算每个像素uv的偏移值  
        float2 offset = dv1  * sinFactor * discardFactor * clamp(_Time.y, 0, 1);
